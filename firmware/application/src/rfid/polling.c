...
void polling_process(void) {
    ...
    if (polling_reader_present()) {
        ...
        // Quick fade animation: visible feedback without blocking the main
        // loop for the full 400ms (polling cadence stays close to interval).
        apply_slot_change_quick(slot_now, slot_new);
    } else {
        m_polling_pending = false;
        if (m_polling_active) {
            ...
            apply_slot_change_quick(slot_now, m_original_slot);
            m_polling_active = false;
        }
    }
}
